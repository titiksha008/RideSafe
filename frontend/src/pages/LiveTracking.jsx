...
// ─── FIX 1: Leaflet default marker icon paths (fixes 404 errors) ───────────
...
export default function LiveTracking() {
  ...
  // ─── FIX 2: Track stopping state to prevent double-clicks ────────────────
  ...
  useEffect(() => {
    ...
    axios
      ...
      .catch((err) => {
        ...
        setLoadError("Could not load ride data.");
      });
  }, [rideId]);
    ...
    const t = setInterval(() => {
      if (rideStartTimeRef.current) {
        setRideTime(
          Math.max(
            0,
            Math.floor((Date.now() - rideStartTimeRef.current) / 1000)
          )
        );
      }
    }, 1000);
  ...
  useEffect(() => {
    ...
    const watchId = navigator.geolocation.watchPosition(
      (pos) => {
        ...
        setSpeed(
          pos.coords.speed ? +(pos.coords.speed * 3.6).toFixed(1) : 0
        );
        // ─── FIX 3: Auth header on location update ─────────────────────────
        axios
          .post(
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        try {
          ...
          setSosMsg(
            `✅ WhatsApp opened + ${
              smsSent ? "📱 SMS sent" : "📱 SMS unavailable"
            } to ${contacts.length} contact(s)`
          );
        } catch (e) {
          setSosMsg(
  ...
  // ─── FIX 4: stopRide — auth header + proper error handling + loading state ─
  const stopRide = async () => {
    ...
    try {
      await axios.post(
        `${API}/rides/stop`,
        { rideId, actualDistance: parseFloat(distCovered) || 0 },
        { headers: { Authorization: `Bearer ${getToken()}` } }
      );
      // Clear any pending timers before leaving
      if (stoppedTimerRef.current) clearTimeout(stoppedTimerRef.current);
      if (sosCountdownRef.current) clearInterval(sosCountdownRef.current);
      navigate("/dashboard");
    } catch (err) {
  ...
  const stationsAhead = policeStations.filter(
    (s) => s.distKm > currentDistKm - 0.2
  );
  const stationsBehind = policeStations.filter(
    (s) => s.distKm <= currentDistKm - 0.2
  );
      ...
      <div className="lt-page">
        ...
        {arrived && (
          <div className="lt-banner lt-banner-arrived">
            🎉 You've arrived!{" "}
            <button onClick={stopRide} disabled={isStopping}>
            ...
            {nextStation && (
              <div className="lt-next-station-card">
                ...
                <div className="lt-next-station-info">
                  <p className="lt-next-station-name">
                    🚔 {nextStation.name}
                  </p>
                  <p className="lt-next-station-dist">
                    {Math.max(
                      0,
                      nextStation.distKm - currentDistKm
                    ).toFixed(1)}{" "}
                    km away
                    {remainingDist && (
            ...
            <div className="lt-station-timeline">
              ...
              {policeStations.map((s, i) => {
                ...
                const isNext =
                  !passed &&
                  i ===
                    policeStations.findIndex(
                      (x) => x.distKm > currentDistKm - 0.2
                    );
                const distFromHere = Math.max(
                  0,
                  s.distKm - currentDistKm
                ).toFixed(1);
                return (
                  <div
                    key={i}
                    className={`lt-timeline-item ${
                      passed ? "lt-timeline-passed" : ""
        ...
        {currentPos && (
          <div className="lt-map-wrapper">
            <div className="lt-safety-legend">
              <div>
                <span className="lg lg-green"></span> Safe &lt;6 km
              </div>
              <div>
                <span className="lg lg-yellow"></span> Moderate 6-11 km
              </div>
              <div>
                <span className="lg lg-orange"></span> Risky 11-16 km
              </div>
              <div>
                <span className="lg lg-red"></span> Unsafe &gt;16 km
              </div>
            </div>
        ...
        <div className="lt-actions">
          ...
          {/* ─── FIX 5: Stop button shows loading state & is disabled while stopping */}
          <button
            className="lt-stop-btn"
            onClick={stopRide}
            disabled={isStopping}
            style={{ opacity: isStopping ? 0.6 : 1 }}
          >
            {isStopping ? "⏳ Ending ride…" : "⏹ Stop Ride"}
          </button>
        </div>
