...
//   const actualTravelDistanceRef = useRef(0);
...
//     lat,
//     lng,
//     actualDistance: parseFloat(actualTravelDistanceRef.current.toFixed(3)) || 0,
//   },
//   { headers: { Authorization: `Bearer ${getToken()}` } }
// ).catch(() => {});
...
//   actualTravelDistanceRef.current += moved / 1000;
//   setActualTravelDistance(actualTravelDistanceRef.current);
// }
...
// How many consecutive off-route GPS pings before triggering AI
const AI_DEVIATION_CONSECUTIVE = 3;
const STATION_LOOKAHEAD_KM = 15;
const STATION_VISIBLE_LIMIT = 5;
...
const stopRide = async () => {
  ...
  const nearbyStations = stationsAhead.filter(
    (s) => s.distKm - currentDistKm <= STATION_LOOKAHEAD_KM
  );

  const visibleStations = showAllStations
    ? stationsAhead
    : (nearbyStations.length > 0 ? nearbyStations : stationsAhead).slice(
        0,
        STATION_VISIBLE_LIMIT
      );

  const nextStation = stationsAhead[0] || null;
  const hiddenStationCount = Math.max(stationsAhead.length - visibleStations.length, 0);
      ...
      <div className="lt-page">
        ...
        {policeStations.length > 0 && (
          <div className="lt-police-panel">
            ...
            <div className="lt-station-filter-row">
              <span>
                {showAllStations
                  ? `Showing all ${stationsAhead.length} upcoming stations`
                  : nearbyStations.length > 0
                  ? `Showing ${visibleStations.length} nearby station(s) within ${STATION_LOOKAHEAD_KM} km`
                  : `Showing next ${visibleStations.length} upcoming station(s)`}
                {!showAllStations && hiddenStationCount > 0
                  ? ` · ${hiddenStationCount} more ahead`
                  : ""}
              </span>

              {stationsAhead.length > STATION_VISIBLE_LIMIT && (
                <button
                  className="lt-station-toggle-btn"
            ...
            <div className="lt-station-timeline">
              ...
              {visibleStations.map((s, i) => {
                ...
                const isNext =
                  !passed &&
                  nextStation &&
                  s.lat === nextStation.lat &&
                  s.lng === nextStation.lng;
                const distFromHere = Math.max(0, s.distKm - currentDistKm).toFixed(1);
                return (
                  <div
                    key={i}
